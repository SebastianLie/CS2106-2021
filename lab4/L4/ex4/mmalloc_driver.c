...
{
    int seed, nReq, heapSize, minSize, maxSize;
  
    if (argc < 6){
        printf("Usage: %s <RandSeed> <HeapSize> <minPartitionSize>"
                "<maxPartitionSize> <Requests#>\n",argv[0]);
        return 1;
    }
...
#ifdef DEBUG
    printf("Seed %d, heap size %d, MinPartSize %d, MaxPartSize %d, #Req %d\n", 
        seed, heapSize, minSize, maxSize,nReq);
#endif
    ...
    //Initialize our "heap"
    if(!setupHeap( heapSize, minSize, maxSize )){
        return 1;
    }

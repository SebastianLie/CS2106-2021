...
{
    int nReq, heapSize,minSize, maxSize;
  
    if (argc != 5){
        printf("Usage: %s <HeapSize> <minPartitionSize)>"
                "<maxPartitionSize> <Active Partitions#>\n",argv[0]);
        return 1;
    }
...
#ifdef DEBUG
    printf("Heap size %d, MinPartSize %d, MaxPartSize %d, Action Partitions %d\n", 
        heapSize, minSize, maxSize,nReq);
#endif
